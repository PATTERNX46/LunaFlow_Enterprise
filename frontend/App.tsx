...
import { Heart, History, ShieldAlert, Plus, LayoutDashboard, LogOut, User as UserIcon, MapPin, Search, Loader2, Trash2, BellRing, Sparkles, ArrowRight, Info, Activity, Zap, BookOpen, Bluetooth, Stethoscope, Video, BookCheck, Calendar, ShieldCheck, AlertTriangle, FileText, Settings } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // 🌟 NEW: Added 'profile' to the active tab options
  const [activeTab, setActiveTab] = useState<'dashboard' | 'history' | 'analysis' | 'tutorials' | 'doctors' | 'assignment' | 'profile'>('dashboard'); 
  ...
  // 🌟 NEW: Function to handle global profile updates from the ProfilePage
            ...
            <>
              <div className="min-h-screen flex flex-col lg:flex-row bg-[#fff5f5] relative overflow-hidden w-full">
                ...
                <nav className={`relative z-10 w-full lg:w-72 bg-white/60 backdrop-blur-xl border-b lg:border-r border-rose-100 lg:h-screen sticky top-0 p-6 flex flex-col shadow-xl transition-opacity duration-500 ${showOverview ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                  {/* New Updated Code with Larger Logo */}
                  <div className="flex items-center gap-4 px-6 py-4">
                     {/* New Larger Logo: Changed w-10 h-10 to w-14 h-14 */}
                     <div className="w-19 h-16 rounded-full overflow-hidden border-2 border-rose-100 shadow-md shrink-0">
                  ...
                  <div className="space-y-2 flex-1 mt-4">
                    ...
                    {showFullBaseApp && (
                      <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl font-bold transition-all mt-2 ${activeTab === 'profile' ? 'bg-rose-500 text-white shadow-lg' : 'text-slate-400 hover:bg-rose-50'}`}>
                ...
                <main className={`relative z-10 flex-1 p-4 lg:p-10 pb-24 lg:pb-10 max-w-7xl mx-auto w-full transition-opacity duration-500 ${showOverview ? 'opacity-0' : 'opacity-100'} overflow-y-auto`}>
                  <header className="flex items-center justify-between gap-8 mb-10"> 
                    <div>
                      <h2 className="text-4xl font-black text-slate-800 capitalize tracking-tight">{activeTab === 'profile' ? 'My Profile' : activeTab}</h2>
                    ...
                    {showFullBaseApp && activeTab !== 'profile' && (
                  ...
                  <div className="space-y-4 mb-8">
                    {showCycleAlarm && showFullBaseApp && activeTab !== 'profile' && (
