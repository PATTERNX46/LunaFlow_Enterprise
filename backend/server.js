...
// 🌟 UPDATED: Added Heart Rate (hrValue) and SpO2 (spo2Value) to the schema
...
// 🌟 UPDATED: Complaint Schema now has an 'evidence' field for images
...
app.post('/api/hb/save', async (req, res) => {
  try {
    // 🌟 UPDATED: Now receiving hrValue and spo2Value as well
    ...
    // 🌟 UPDATED: Save all three metrics to MongoDB
...
// 13. GENERATE COMBINED ANEMIA + PERIOD REPORT 
app.get('/api/anemia-combo/:userId', async (req, res) => {
  try {
    ...
    const pythonResponse = await axios.post('http://127.0.0.1:8000/predict_anemia_combo', {
